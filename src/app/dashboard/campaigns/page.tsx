...
import { Plus, Trash2, Edit, ExternalLink, Smartphone, Calendar, Copy, Check } from "lucide-react";
...
export default function CampaignListPage() {
    ...
    const handleCopy = (e: React.MouseEvent, campaign: Campaign) => {
        e.stopPropagation();
        ...
        <div className="space-y-6 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {(campaigns as Campaign[] | undefined)?.map((campaign) => {
                    const campaignUrl = campaign.slug ? `/c/${campaign.slug}` : `/campaign/${campaign._id}`;
                        ...
                        >
                            ...
                            <div className="aspect-[9/16] bg-gray-50 flex items-center justify-center relative overflow-hidden border-b border-gray-50">
                                ...
                                <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
                                    <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black tracking-tight shadow-sm w-fit ${campaign.status === "published"
                                    ...
                                    {campaign.slug && (
                                        <span className="bg-blue-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-black tracking-tight shadow-sm w-fit">
                                            {campaign.slug}
                                        </span>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center justify-between pt-2 border-t border-gray-50 mt-auto">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="p-2 text-gray-400 hover:text-blue-600 transition-colors tooltip"
                                            title="URL 복사"
                                        >
                                            {copiedId === campaign._id ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                                        </button>
                                        <Link
                                            href={campaignUrl}
                                            target="_blank"
